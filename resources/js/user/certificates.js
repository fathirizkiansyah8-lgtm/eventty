            ...
            if (!certificates || certificates.length === 0) {
                container.innerHTML = `
                    <div class="empty-state" style="grid-column:1/-1;text-align:center;padding:3rem;">
                        <iconify-icon icon="solar:medal-ribbons-star-linear" width="44" height="44" style="margin-bottom:1rem;"></iconify-icon>
                ...
                <div class="certificate-card">
                    ...
                    <div class="cert-content">
                        ...
                        <div class="cert-details">
                            <span><iconify-icon icon="solar:calendar-linear"></iconify-icon> ${cert.event_date}</span>
                            <span><iconify-icon icon="solar:medal-star-linear"></iconify-icon> ${cert.issued_date}</span>
                            <span><iconify-icon icon="solar:card-linear"></iconify-icon> ${cert.certificate_number}</span>
                        </div>
                    </div>
                    <div class="cert-actions">
                        ...
                        ${cert.certificate_url
                            ? `<a href="${cert.certificate_url}" class="btn btn-primary btn-sm" download>
                                   <iconify-icon icon="solar:download-linear"></iconify-icon> Unduh
                               </a>`
        ...
        try {
            ...
            content.innerHTML = `
                <div style="text-align:center;padding:1.5rem;">
                    <iconify-icon icon="solar:medal-ribbons-star-linear" width="32" height="32" style="margin-bottom:.5rem;"></iconify-icon>
                    ...
                    <p style="font-size:.78rem;color:#64748b;"><iconify-icon icon="solar:calendar-linear"></iconify-icon> ${cert.event_date} &nbsp;|&nbsp; <iconify-icon icon="solar:medal-star-linear"></iconify-icon> Diterbitkan: ${cert.issued_date}</p>
                    ...
                    ${cert.certificate_url ? `<a href="${cert.certificate_url}" class="btn btn-primary" style="margin-top:1rem;display:inline-block;" download><iconify-icon icon="solar:download-linear"></iconify-icon> Download PDF</a>` : ''}
                </div>
            `;
